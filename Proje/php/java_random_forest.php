...
<html>
<head>
<title>Java Random Forest Tablosu</title>
...
$sql = "SELECT 1class, 2class, 3class, 4class, 5class, 6class FROM java_random_forest";
$result = $conn->query($sql);
if ($result->num_rows > 0) {
// output data of each row
while($row = $result->fetch_assoc()) {
echo "<tr><td>" . $row["1class"]. "</td><td>" . $row["2class"] . "</td><td>"  . $row["3class"] . "</td><td>" . $row["4class"] . "</td><td>". $row["5class"] . "</td><td>"
. $row["6class"]. "</td></tr>";
}
echo "</table>";
} else { echo "0 results"; }
