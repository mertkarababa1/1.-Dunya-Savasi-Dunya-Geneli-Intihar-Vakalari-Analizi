...
$sql = "SELECT tpos, fpos, tneg, fneg, recl, prec, sens, spec, fmes, acur, cohen FROM naive_bayes";
$result = $conn->query($sql);
if ($result->num_rows > 0) {
// output data of each row
while($row = $result->fetch_assoc()) {
    echo "<tr><td>" . $row["tpos"]. "</td><td>" . $row["fpos"] . "</td><td>" . $row["tneg"] . "</td><td>" . $row["fneg"] . "</td><td>" . $row["recl"] . "</td><td>" . $row["prec"] . "</td><td>" . $row["sens"] . "</td><td>"  . $row["spec"] . "</td><td>" . $row["fmes"] . "</td><td>" . $row["acur"] . "</td><td>"
    . $row["cohen"]. "</td></tr>";
}
echo "</table>";
} else { echo "0 results"; }
